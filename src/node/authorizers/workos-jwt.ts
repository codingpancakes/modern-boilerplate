...
export const handler = async (
  ...
  console.log('🔑 Auth header:', auth ? `Bearer ${auth.slice(0, 20)}...` : 'missing');

  const token = auth.startsWith('Bearer ')
    ? auth.slice('Bearer '.length).trim()
    : '';

  console.log('Extracted token:', token ? `${token.slice(0, 20)}...${token.slice(-20)}` : 'missing');
  ...
  try {
    ...
    const verifyPromise = jwtVerify(token, JWKS, {
      issuer: [AUTH_ISSUER, `https://api.workos.com/user_management/${CLIENT_ID}`], // Accept both issuers like local
    ...
    const reason =
      err instanceof errors.JWTExpired
        ? 'token_expired'
        : 'invalid_token';
    console.error('❌ JWT verification failed:', { reason, error: (err as Error).message, clientId: CLIENT_ID, issuer: AUTH_ISSUER });
    return { isAuthorized: false };
  }
  } catch (globalErr) {
