...
> entire cutover was executed at once on a temporary feature branch: no Cloudflare-proxies-to-AWS
...
> `wrangler dev --local` plus the unit/integration suites and live smoke probes. The
> production repo now uses only `staging` and `main` as deploy branches.
>
> | Phase | Status |
> |---|---|
> | 1 — Hono unification | ✅ Done (one app, `src/node/app.ts`; Express shim deleted) |
> | 2 — Cloudflare foundations | ✅ wrangler.toml + envs, secrets sync, R2 bindings, GitHub Actions CI · ⬜ Hyperdrive, real per-project R2 buckets after `init-project` |
> | 3 — Cutover | ✅ All routes, GraphQL Yoga, Cron Triggers, structured logger, Queues + webhook DLQ |
> | 4 — Operational shell | ✅ Gradual deploy + auto-rollback (`scripts/deploy.ts`), CI/CD branch mapping, runtime audit gate, post-deploy smoke checks · ⬜ Dashboard alert evidence and optional Logpush |
> | 5 — Decommission AWS | ✅ Code purged from this branch; current docs updated · ⬜ Actual AWS account teardown ([legacy guide](../legacy-aws/CDK_TEARDOWN.md), run from legacy history) |
>
...
> S3 API via `aws4fetch`. ✅ GitHub Actions CI is in place. ⬜ Hyperdrive remains
> optional; the Neon serverless driver is used directly for now. Fresh boilerplate
> projects still need to create the R2 buckets generated by `pnpm init-project`.
...
> `src/node/lib/logger.ts`. ✅ Cloudflare Queues now backs webhook processing, with a
> DLQ consumer that reports to Sentry and writes `WEBHOOK_FAILED` audit rows.
...
> canary + auto-rollback; `deploy:*:simple` keeps the all-at-once path. ✅ CI/CD:
> `.github/workflows/ci.yml` gates pull requests into `staging` and `main`, deploys
> staging on `staging`, and starts production deployment on `main` behind GitHub
> environment approval. ⬜ Remaining: dashboard alert evidence and optional Logpush
> retention sink.

### Phase 5 — Decommission AWS — ✅ CODE / ⬜ ACCOUNT

- Follow [CDK_TEARDOWN](../legacy-aws/CDK_TEARDOWN.md) from legacy history; the CDK
  code no longer exists on this branch. Archive
